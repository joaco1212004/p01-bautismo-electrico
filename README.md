...
2. Medir el riel de **3.3 V** de la placa → anotar: `V_3V3 = 3.317`
3. Medir el riel de **5 V** (VBUS del USB) → anotar: `V_5V = 5.068`
...
3. Anotar: nominal = 1000 Ω · medido = `998` Ω · error = `0.2` %

> ¿Qué tolerancia tiene? ¿El valor medido cae dentro de esa tolerancia? 
Al tener un 5% de error, y tener un valor de 1 kΩ, segun el fabricante, la tolerancia esta entre +- 50Ω por lo que la tolerancia es entre 950Ω y 1050Ω y en este caso, el valor cae en 998Ω por lo que el valor medio cae dentro de esta tolerancia.

### Actividad 5 — Calcular y armar circuito LED + R

Queremos hacer circular **20 mA** por un LED rojo (V~F~ ≈ 2.0 V) con V~CC~ = 3.3 V.

$$R = \frac{V_{CC} - V_F}{I_F} = \frac{3.3\ \text{V} - 2.0\ \text{V}}{20\ \text{mA}} = 65Ω
...
1. Medir **V~R~** (caída en la resistencia): colocar puntas del multímetro en ambos extremos de R -> 1.294V
2. Medir **V~LED~** (caída en el LED) -> 1.999 V
3. Verificar: V~R~ + V~LED~ ≈ 3.3 V -> 1.294V + 1.999V = 3.293V
4. Calcular la corriente real: **I = V~R~ / R** -> 1.294V / 66.6Ω = 0.0194A

Anotar:
- `V_R = 1.294` V
- `V_LED = 1.999` V
- `I_real = 0.0194` mA (¿razonable para un LED encendido?)
...
`.text` ocupa 332 bytes y, `.data` y `.bss` 0 bytes 
...
Si es 50% duty cicle y la señal no tiene glitches.
...
- Período medido: `998.89` ms
- Delta: `δ = (f_medida - 1.0) / 1.0 × 100 = 0` % -> La frecuencia que nos muestra el osciloscopio esta limitada a 2 unidades decimales, por eso el delta se redondea a 0
...
El HSI es el oscilador interno que genera un clock de aproximadamente 8 MHz. Según el datasheet, este valor no es perfectamente exacto y puede variar por factores como la temperatura y la tensión de alimentación. Como el timer usa este clock como referencia, si el HSI no está funcionando exactamente a 8 MHz, la frecuencia medida también va a ser distinta de la esperada.
...
| Medición | Valor |
|----------|-------|
| V riel 3.3 V | 3.317 V |
| V riel 5 V | 5.068 V |
| Resistencia nominal | 1000 Ω |
| Resistencia medida | 998 Ω |
| V_R (caída en R del LED) | 1.294 V |
| V_LED (caída en LED) | 1.999 V |
| I real (calculada) | 0.0194 mA |
| Frecuencia configurada | 1 Hz |
| Frecuencia medida | 1 Hz |
| Delta (%) | 0% (osciloscopio mimde en 2 unidades decimales)|
